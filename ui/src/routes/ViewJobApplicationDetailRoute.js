...
const ViewJobApplicationDetailRoute = (props) =>{
    ...
    const [ data, setData ] = useState({ jobTitle: '', locationType: '--', locationSupport: '--', workLocation: '--', companyName: '', companyLink: '', companySummary: '', companyLogo: '', paymentType: '', workType: '', jobDesc: '', searchTerms: '', applyLink: '', skills: [], searchCategory: [], postedDate: null });
    ...
    const getDetail = useCallback(async() =>{
        ...
        setData({
            jobTitle: result.jobTitle, locationType: result.locationType, locationSupport: result.locationSupport, workLocation: result.workLocation, companyName: result.companyName, companyLink: result.companyLink, companySummary: result.companySummary, companyLogo: result.companyLogo, paymentType: result.paymentType, workType: result.workType, jobDesc: result.jobDesc, searchTerms: result.searchTerms, applyLink: result.applyLink, skills: result.skills, searchCategory: result.searchCategory,postedDate: result.postedDate
        });
    },[]);
    ...
    const trim = (value, length) =>{
        if(length === 1)return value.slice(0,1);
        if(!isNull(value)){
    ...
    const copyHandler = () =>{
        if(!isNull(data.applyLink)){
            navigator.clipboard.writeText(data.applyLink).then(()=>{
                setIsCopied(true);
                ...
                <div className={classes.backwardIconContainer}>
                    ...
                    <div className={classes.jobTitleColoredDivLeft}>
                        ...
                        <div className={classes.jobTitleNameContainer}>
                            <span>
                                {isNull(data.companyLogo) && trim(data.companyName, 1)}
                                {/* {!isNull(data.companyLogo) && <img src={URL.createObjectURL(data.companyLogo)} alt="" />} */}
                                </span>
                ...
                <footer className={classes.footer}>
                    ...
                    <div className={classes.footerDiv}>
                        ...
                        <span className={classes.list}>
                            ...
                            {(!isNull(data.applyLink) && !isCopied) &&<img src={copyIcon} alt='' className={classes.copyIcon} onClick={copyHandler} />}
                           {(!isNull(data.applyLink) && isCopied) && <img src={checkIcon} alt='' className={classes.checkIcon} />}
                        </span>
                    </div>
                    <div className={classes.footerDiv}>
                        <h1>First Posted: </h1>
